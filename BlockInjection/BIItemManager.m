...
- (BIItem *)itemForMethodName:(NSString *)methodName forClass:(Class) class {
  return [self.items
      objectForKey:[self keyForMethodName:methodName forClass:class]];
}

    - (void)setItem : (BIItem *)item forMethodName
                      : (NSString *)methodName forClass : (Class) class {
  [self.items setObject:item
                 forKey:[self keyForMethodName:methodName forClass:class]];
}

    - (void)removeItemForMethodName : (NSString *)methodName forClass
                                      : (Class) class {
...
- (NSString *)keyForMethodName:(NSString *)methodName forClass:(Class) class {
  return [NSString
      stringWithFormat:@"%@::%@", NSStringFromClass(class), methodName];
}
