...
#define REPLACEBLOCK_FOR(type)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
    ...
    memcpy(dest, retp, sizeof(type));                                          \
    va_end(argp);                                                              \
    return *dest; \
    }

#define REPLACE_BLOCK_FOR_STRUCT_CASE(cas) \
    case cas:                              \
        return REPLACEBLOCK_FOR(BILibStruct##cas)
...
{
    return [BILib injectToSelector:sel
                          forClass:class
...
+ (BOOL)injectToClassWithNames:(NSArray *)classNames
    ...
    for (NSString *className in classNames) {
        for (NSString *methodName in methodNames) {
            failed |= ![BILib injectToClassWithName:className
...
+ (BOOL)injectToClassWithNameRegex:(NSRegularExpression *)classNameRegex
    ...
    for (NSValue *classValue in matchClasses) {
        ...
        NSArray *matchSelectors =
            [BILibUtils selectorsWithRegex:methodNameRegex forClass:class];
...
+ (BOOL)injectToClassWithNameRegex:(NSRegularExpression *)classNameRegex
    ...
    for (NSValue *classValue in matchClasses) {
        ...
        for (NSValue *selValue in matchSelectors) {
            SEL sel = [selValue pointerValue];
            failed |=
                ![BILib injectToClass:class selector:sel postprocess:postprocess];
        }
    }
    return !failed;
}
...
+ (BOOL)replaceImplementationForClass:
    ...
    + (BOOL)replaceImplementationForClassName : (NSString *)className methodName
                                                : (NSString *)methodName block
                                                  : (id)block {
    Class class = objc_getClass([className UTF8String]);
    SEL sel = sel_getUid([methodName UTF8String]);
...
+ (BOOL)injectToSelector:(SEL)sel
    ...
    @try {
        ...
        Method originalMethod = [BILibUtils getMethodInClass:class
                                                    selector:sel
        ...
        BIItem *item = [[BIItemManager sharedInstance] itemForMethodName:methodName
                                                                forClass:class];
        if (!item) {
            @try {
                [NSMethodSignature
                    signatureWithObjCTypes:method_getTypeEncoding(originalMethod)];
            }
            ...
            item.targetSel = sel;
            item.originalSel = sel_registerName(
                [[BILibUtils saveNameForMethodName:methodName] UTF8String]);
            item.originalMethod = originalMethod;
            item.numberOfArguments = method_getNumberOfArguments(originalMethod) - 2;
            item.signature = [NSMethodSignature
                signatureWithObjCTypes:method_getTypeEncoding(originalMethod)];
            item.isClassMethod = isClassMethod;

            [BILib replaceImplementationWithItem:item];

            [[BIItemManager sharedInstance] setItem:item
                                      forMethodName:methodName
                                           forClass:class];
        }
        [BILib savePreprocess:preprocess
               andPostprocess:postprocess
                     withItem:item
                forMethodName:methodName];
    }
    @catch (NSException *exception) {
...
+ (void)savePreprocess:(id)preprocess
        andPostprocess:(id)postprocess
              withItem:(BIItem *)item
         forMethodName:(NSString *)methodName {
    if (preprocess) {
        // Save preprocess
        SEL preprocessSel = sel_registerName([[BILibUtils
            preprocessNameForClassName:NSStringFromClass(item.targetClass)
                            methodName:methodName
    ...
    if (postprocess) {
        // Save postprocess
        SEL postprocessSel = sel_registerName([[BILibUtils
            postprocessNameForClassName:NSStringFromClass(item.targetClass)
                             methodName:methodName
    ...
    if (item.signature) {
        ...
        if (NULL == returnType || 0 == strlen(returnType)) {
            ...
            switch (type) {
                ...
                case 'c': {
                    replaceBlock = REPLACEBLOCK_FOR(char);
                } break;
                case 'i': {
                    replaceBlock = REPLACEBLOCK_FOR(int);
                } break;
                case 's': {
                    replaceBlock = REPLACEBLOCK_FOR(short);
                } break;
                case 'l': {
                    replaceBlock = REPLACEBLOCK_FOR(long);
                } break;
                case 'q': {
                    replaceBlock = REPLACEBLOCK_FOR(long long);
                } break;
                case 'C': {
                    replaceBlock = REPLACEBLOCK_FOR(unsigned char);
                } break;
                case 'I': {
                    replaceBlock = REPLACEBLOCK_FOR(unsigned int);
                } break;
                case 'S': {
                    replaceBlock = REPLACEBLOCK_FOR(unsigned short);
                } break;
                case 'L': {
                    replaceBlock = REPLACEBLOCK_FOR(unsigned long);
                } break;
                case 'Q': {
                    replaceBlock = REPLACEBLOCK_FOR(unsigned long long);
                } break;
                case 'f': {
                    replaceBlock = REPLACEBLOCK_FOR(float);
                } break;
                case 'd': {
                    replaceBlock = REPLACEBLOCK_FOR(double);
                } break;
                case 'B': {
                    replaceBlock = REPLACEBLOCK_FOR(bool);
                } break;
                case '*': {
                    replaceBlock = REPLACEBLOCK_FOR(int *);
                } break;
                ...
                case '{': {
                    replaceBlock =
                        [BILib replaceBlockForStructWithSize:returnLength withItem:item];
                } break;
                case '^': {
                    replaceBlock = REPLACEBLOCK_FOR(int *);
                } break;
...
+ (id)replaceBlockForStructWithSize:(NSUInteger)size withItem:(BIItem *)item {
    if (size == 16) {
      ...
      CGPoint *dest = malloc(sizeof(CGPoint));
      memcpy(dest, retp, sizeof(CGPoint));

      NSLog(@"End F1 : %@", NSStringFromCGPoint(*(CGPoint *)retp));
      NSLog(@"End F2 : %@", NSStringFromCGPoint(*dest));
      return *dest;
        };
    } else {
        switch (size) {
