...
- (void)testHandlePrettyFunction {
  ...
  STAssertTrue([handlePrettyFunction
                   isEqualToString:@"-[SubjectForRegex2 instanceMethod:]"],
               @"handlePrettyFunction is invalid: %@", handlePrettyFunction);

  [[SubjectForRegex new] instanceMethod:@"hello!"];

  STAssertTrue([handlePrettyFunction
                   isEqualToString:@"-[SubjectForRegex instanceMethod:]"],
               @"handlePrettyFunction is invalid: %@", handlePrettyFunction);

  [[SubjectForRegex new] instanceMethod2];

  STAssertTrue([handlePrettyFunction
                   isEqualToString:@"-[SubjectForRegex instanceMethod2]"],
               @"handlePrettyFunction is invalid: %@", handlePrettyFunction);
}
  ...
  NSRegularExpression *classNameRegex =
      [NSRegularExpression regularExpressionWithPattern:@"^UIView$"
                                                options:0
                                                  error:&error];
  NSRegularExpression *methodNameRegex =
      [NSRegularExpression regularExpressionWithPattern:@"^set.*$"
                                                options:0
                                                  error:&error];
